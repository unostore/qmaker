...
constellation-node node1.conf 2>> qdata/constellationLogs/constellation_node1.log &
sleep 1
...
PRIVATE_CONFIG=qdata/node1.ipc geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpccorsdomain "*" --raftport $RA_PORT --rpcport $R_PORT --port $W_PORT --ws --wsaddr 0.0.0.0 --wsport $WS_PORT --wsorigins '*' --wsapi --nat extip:$CURRENT_NODE_IP 2>>qdata/gethLogs/node1.log &
